...
DB_CONFIG = {
    'dbname': 'fuel_calc',
    'user': 'postgres',
    'password': '12345',  # In production, use environment variable: os.getenv('DB_PASSWORD')
    'host': 'localhost',
    'port': 5432
}
...
class FuelApp(tk.Tk):
    ...
    def _build_calculator(self):
        ...
        img_container = tk.Frame(left, bg=t["bg2"], width=CAR_PHOTO_WIDTH, height=CAR_PHOTO_HEIGHT)
    ...
    def _build_calc_form(self):
        ...
        def make_row(parent, label_text, var, unit, icon):
            ...
            entry = tk.Entry(row, textvariable=var, font=("Courier", 13),
    ...
    def _set_calc_car_image(self, photo_path):
        img = load_car_image(photo_path, CAR_PHOTO_WIDTH, CAR_PHOTO_HEIGHT)
    ...
    def _on_car_selected(self, event=None):
        ...
        if sel in [TR("no_car"), "— No car —"] or not self.current_user or not self._db_connected:
            self._set_calc_car_image(None)
    ...
    def _calculate(self):
        ...
        if self.current_user and self._db_connected:
            try:
                ...
                car_name = self.calc_car_var.get()
                if car_name not in [TR("no_car"), "— No car —"]:
                    db_update_car_consumption(self.current_user, car_name, round(consumption, 2))
    ...
    def _draw_fuel_chart(self):
        ...
        values = [v for _, v in daily_avg if v is not None]
        if not values:
            c.create_text(W // 2, H // 2, text=TR("no_history_chart"),
                          fill=t["fg2"], font=("Courier", 12))
            return
        ...
        # Draw colored zones
        if 8 > min_v and 8 < max_v:
            yg = pt + (1 - (8 - min_v) / rng) * ch
            if yg and yg > pt and yg < H - pb:
                c.create_rectangle(pl, pt, W - pr, yg, fill="#1a2f1a", outline="")
        if 12 > min_v and 12 < max_v:
            yy = pt + (1 - (12 - min_v) / rng) * ch
            if yy and yy > pt and yy < H - pb:
                c.create_rectangle(pl, max(yg, pt) if 'yg' in locals() else pt, W - pr, yy, fill="#2a2410", outline="")
                c.create_rectangle(pl, yy, W - pr, H - pb, fill="#2a1212", outline="")
        ...
        def reset_pw():
            ...
            try:
                if not db_user_exists(email):
                    status.configure(text="Email не найден" if current_language == "ru" else "Email not found")
                    return
                if len(pw) < 6:
                    status.configure(text=TR("password_short"))
                    return
    ...
    def _add_car_inline(self):
        ...
        try:
            ...
            if car_count >= MAX_CARS_PER_USER:
        ...
        preview_box = tk.Frame(photo_preview_frame, bg=t["bg3"], width=CAR_PREVIEW_WIDTH, height=CAR_PREVIEW_HEIGHT)
        ...
        def choose_photo():
            ...
            if path:
                ...
                try:
                    ...
                    prev_img = load_car_image(dest, CAR_PREVIEW_WIDTH, CAR_PREVIEW_HEIGHT)
    ...
    def _render_cars(self):
        ...
        for row_idx in range(5):
            ...
            for col_idx in range(5):
                ...
                img_box = tk.Frame(card, bg=t["bg2"], width=CAR_PREVIEW_WIDTH, height=CAR_PREVIEW_HEIGHT)
                img_box.pack_propagate(False)
                img_box.pack()
                ph = car.get("photo")
                img = load_car_image(ph, CAR_PREVIEW_WIDTH, CAR_PREVIEW_HEIGHT)
                photo = ImageTk.PhotoImage(img)
                ...
                avg = car.get("avg_consumption")
                if avg:
                    tk.Label(card, text=f"💧 {avg:.1f} л/100", font=("Courier", 7),
    ...
    def _build_settings(self):
        ...
        outer, inner = make_scrollable(f, t["bg"])
        outer.pack(fill="both", expand=True)
        self.settings_pad = tk.Frame(inner, bg=t["bg"])
        self.settings_pad.pack(fill="both", expand=True, padx=48, pady=32)
        self._refresh_settings_display()
    ...
    def _refresh_settings_display(self):
        ...
        # Theme settings
        tc = tk.Frame(self.settings_pad, bg=t["bg2"], pady=16)
        ...
        # Currency settings
        cc = tk.Frame(self.settings_pad, bg=t["bg2"], pady=16)
        ...
        # Language settings
        lc = tk.Frame(self.settings_pad, bg=t["bg2"], pady=16)
